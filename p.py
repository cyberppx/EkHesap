...
async def main_page():
    ...
    <script>
        ...
        async function initAds() {
            try {
                ...
                // 1. BANNER REKLAMI (HEP GÖZÜKÜR)
                AdMob.showBanner({
                    adId: BANNER_ID,
                    adSize: BannerAdSize.BANNER,
                    position: BannerAdPosition.BOTTOM_CENTER,
                    margin: 85, // Dock'un üzerine binmemesi için
                    isTesting: true
                });

                // 2. GEÇİŞ REKLAMI MANTIĞI (7-14.3 sn arası bir kez çıkar)
                const delay = (Math.random() * (14.3 - 7) + 7) * 1000;
                console.log("Reklam şu kadar süre sonra çıkacak: " + delay + "ms");
                ...
                console.log("Mobil ortam bulunamadı, reklamlar pasif.");
            }
        }
    ...
    def dev_view():
        with ui.column().classes('w-full items-center text-center gap-6 pt-10'):
            ...
            ui.label(f'{PROJE_ISMI} v7.5').classes('text-3xl font-black text-white')
